...
- [ ] **PIN + biometric unlock + dormant auto-lock (P1, large).** First open uses passphrase (existing). Subsequent opens use a 4–6 digit PIN **or biometric (Face ID / Touch ID / Windows Hello / Android fingerprint)** — biometric is offered as a one-tap alternative to typing the PIN, both unlock the same PIN-wrapped vault key. Forgot PIN/biometric → passphrase fallback. Forgot both → recovery key (existing). Vault auto-locks after a configurable dormant timeout (no decryption events for N minutes), scrubbing the in-memory `cryptoKey`. Triggers PIN/biometric prompt on next decryption. Required on: opening Vault directly; clicking an encrypted entry from MemoryView / EntriesView / DetailModal; chat references to encrypted entries; CaptureModal "Save to Vault" flow. Crypto: derive a wrapper key from the PIN (PBKDF2), encrypt the master vault key with the wrapper, store the wrapped blob in localStorage/IndexedDB. Biometric path uses WebAuthn (platform authenticator) on web and Capacitor BiometricAuth plugin on native shell — both gate decryption of the same wrapped blob via a hardware-backed credential. UI: PIN entry screen, PIN+biometric enrollment screen (after first passphrase unlock), "Use PIN instead" link, "PIN forgotten → passphrase" link, lock-timeout setting (5/15/30/60 min), biometric toggle in Settings → Privacy. Estimate ~500-750 LOC across `lib/crypto.ts`, `lib/biometric.ts` (new — WebAuthn + Capacitor wrapper), `useVaultOps.ts`, `VaultGate.tsx`, `DetailModal.tsx`, plus new `VaultPinScreen.tsx` and `useVaultLockTimer.ts`.
