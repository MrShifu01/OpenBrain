...
### Thu 2026-05-14 — Cmd+K capture + week retro

**Goal:** Power users hit Cmd+K, capture sheet opens in <100ms with focus in the textarea. Plus a one-page retro on what shipped.
...
- 14:00–15:00 — Mobile fallback: external bluetooth keyboards on iPad. Same handler, no special-case.
- 15:00–17:00 — Manual test: 20 captures via Cmd+K back-to-back. Confirm no listener leaks, no focus glitches.
...
**Commit pattern:** `feat(cmd-k): global capture shortcut + retro` (two commits — one for the feature, one for the retro doc)
