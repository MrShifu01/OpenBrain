...
- [ ] **Tooltip** — currently <5 native `title` attrs. Add tooltips to icon buttons across header, bulk bar, settings. Low-priority polish.
- [ ] **Accordion** — 6 files have `[expanded, setExpanded]` patterns (`AdminTab`, `BulkActionBar`, `NotificationBell`, `OnboardingModal`, `OmniSearch`, `ChatDebugPanel`); most are stateful menus rather than collapsible content. Real Accordion candidates limited — defer until a section actually demands collapsible animation.
- [ ] **Calendar + Popover** — replace 3 native `<input type="date">` instances in `TodoCalendarEvent` (event editor) + `ScheduleInline`.
