...
| **Maintainability**    | 8.4 / 10 | TypeScript strict, 11 e2e specs, 450 unit tests passing, Vitest + Playwright separated cleanly, CLAUDE.md + RUNBOOK.md present. ESLint warning ratchet at **73** (down from 297, -224, -75%). Drag: 6 `as any` casts in src/ (all legit ESM-compat or annotated Supabase quirks), one god-component >1000 lines remaining (EntryList) — TodoCalendarTab split 1576 → 430 + 3 sibling files; VaultView split 1207 → 101 (router) + 4 sibling files (Loading / Setup / Gate / Unlocked).                                                                  |
...
- [ ] **One god-component >1000 lines remaining** 🟡
      `src/components/EntryList.tsx` (986). ✅ `src/views/TodoCalendarTab.tsx` (was 1576) split into 4 files: orchestrator 430, `todoCalendarHelpers.ts` 244, `TodoCalendarChrome.tsx` 529, `TodoCalendarEvent.tsx` 480. ✅ `src/views/VaultView.tsx` (was 1207) split into 5 files: router 101, `VaultLoading.tsx` 184, `VaultSetup.tsx` 184, `VaultGate.tsx` 346, `VaultUnlocked.tsx` 540. EntryList handles fetching, filtering, display, and mutations in one file. Extract presentational sub-components (e.g., `EntryListFilters`) and move side effects into hooks. Don't refactor for its own sake — do it next time you touch it for a feature.
