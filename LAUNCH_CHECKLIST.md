...
| **Maintainability**    | 8.2 / 10 | TypeScript strict, 11 e2e specs, 450 unit tests passing, Vitest + Playwright separated cleanly, CLAUDE.md + RUNBOOK.md present. ESLint warning ratchet at **73** (down from 297, -224, -75%). Drag: 6 `as any` casts in src/ (all legit ESM-compat or annotated Supabase quirks), two god-components >1000 lines remaining (VaultView, EntryList) — TodoCalendarTab split 1576 → 430 (orchestrator) + 3 sibling files (helpers, chrome, event).                                                                  |
...
- [ ] **Two god-components >1000 lines remaining** 🟡
      `src/views/VaultView.tsx` (1217), `src/components/EntryList.tsx` (986). ✅ `src/views/TodoCalendarTab.tsx` (was 1576) split into 4 files: orchestrator 430, `todoCalendarHelpers.ts` 244, `TodoCalendarChrome.tsx` 529, `TodoCalendarEvent.tsx` 480. Each handles fetching, filtering, display, and mutations in one file. Extract presentational sub-components (e.g., `EntryListFilters`, `VaultEntryCard`) and move side effects into hooks. Don't refactor for its own sake — do it next time you touch one of these for a feature.
