...
export async function setupVault(passphrase: string): Promise<{ key: CryptoKey; salt: string; verifyToken: string }> {
...
export async function unlockVault(passphrase: string, saltHex: string, verifyToken: string): Promise<CryptoKey | null> {
...
export async function encryptVaultKeyForRecovery(vaultKey: CryptoKey, recoveryKey: string): Promise<string> {
...
export async function decryptVaultKeyFromRecovery(recoveryBlob: string, recoveryKey: string): Promise<CryptoKey | null> {
  try {
    ...
    return await crypto.subtle.importKey(
      "raw", rawKeyBytes, { name: CIPHER, length: 256 }, true, ["encrypt", "decrypt"]
    );
  } catch {
    return null;
  }
...
export async function encryptEntry(entry: EncryptableEntry, cryptoKey: CryptoKey): Promise<EncryptableEntry> {
  const out = { ...entry };
...
export async function decryptEntry(entry: EncryptableEntry, cryptoKey: CryptoKey): Promise<EncryptableEntry> {
  const out = { ...entry };
