...
import { verifyPin, getStoredPinHash } from "../lib/pin";

type Stage = "pin" | "busy" | "revealed" | "no-cache" | "error";
...
export default function VaultRevealModal({
  ...
  const [stage, setStage] = useState<Stage>(canReveal ? "pin" : "no-cache");
  ...
  useEffect(() => {
    if (stage === "pin") setTimeout(() => inputRef.current?.focus(), 60);
  }, [stage]);
  ...
  async function handleSubmit() {
    ...
    if (!stored) { setPinError("No PIN set — go to Settings to create one"); doShake(); return; }
    setStage("busy");
      ...
      >
        ...
        >
          ...
          <div className="mb-5 text-center">
            <div className="mb-2 text-[28px]">{stage === "revealed" ? "🔓" : "🔒"}</div>
            <h3
              className="m-0 text-[17px] font-bold"
              style={{ color: "var(--color-on-surface)" }}
            >
              {stage === "revealed" ? entry.title : "Secret"}
            </h3>
            {stage !== "revealed" && (
            ...
            <>
              ...
              <input
                ...
                style={{
                  ...
                  border: `1px solid ${pinError ? "var(--color-error)" : "var(--color-outline-variant)"}`,
